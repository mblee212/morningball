...
## v7.3 정합성 회귀 수정 (선발 사라짐·상대전적·날씨100% 3대 버그)
근본 원인: batch1의 선발·record·상대전적이 하나의 거대 try에 묶여, 중간 에러 하나가 전부를 스킵시킴.
- 선발/구장/record/상대전적을 각각 독립 try로 분리 → 하나 실패해도 나머지 정상
- 선발은 이름·라인(2중 분리)과 지표 카드를 다시 분리 → 지표 계산 실패해도 이름은 반드시 표시
- 상대전적 h2h는 조회팀 code 기준 유지 확인 (SSG→KIA 3-6, LG→KT 6-3 실검증)
- 날씨 버그: buildPlayable이 데이터 전무 시 score=100 유지하던 것 → playable:null(미확정)로 변경
  * 프런트 게이지 null-safe(— 표시), 서버·프런트 검증기 playable null 허용
- 회귀 확인: 날씨 정상시 숫자 정상 산출(강수30%→79), 휴식일 0, 검증 통과