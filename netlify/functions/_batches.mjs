...
import { TEAMCODE, getGames, findGame, getPreview, getSeasonGames, computeH2H, pick, num, str, s2, starterLine, mapBatter } from "./collectors/naver.mjs";
...
import { computePitcher } from "./collectors/compute.mjs";

const SEASON_START = () => `${kstNow().getUTCFullYear()}-03-01`;
...
export async function runBatch1() {
  ...
  /* 시즌 전체 일정 1회 수집 → 모든 팀의 상대전적 집계에 재사용 */
  let seasonGames = null;
  try { seasonGames = await getSeasonGames(SEASON_START(), today); sources.season = "ok"; }
  catch (e) { sources.season = "keep-prev: " + e.message; }
  ...
  for (const teamKey of Object.keys(TEAMCODE)) {
    ...
    const myHome = game.homeTeamCode === code;
    const oppCode = myHome ? game.awayTeamCode : game.homeTeamCode;
    const oppName = str(myHome ? game.awayTeamName : game.homeTeamName, "상대팀");
    const hhmm = (String(game.gameDateTime || "").match(/T(\d{2}:\d{2})/) || [])[1] || "18:30";
    const stadium = str(game.stadium, "");

    T.game.opp = oppName;
    T.game.oppShort = oppName.slice(0, oppName.includes(" ") ? oppName.indexOf(" ") : 2);
    T.game.time = hhmm;
    T.game.venue = `${stadium || "구장 미정"} (${myHome ? "홈" : "원정"})`;
    T.game.meta = `${dateLabel()} · ${myHome ? "홈" : "원정"} 경기`;

    /* 선발투수 (프리뷰) — 같은 경기 상대팀과 공유 */
    try {
      if (!(game.gameId in previewCache)) previewCache[game.gameId] = await getPreview(game.gameId);
      const pv = previewCache[game.gameId];
      const myP = myHome ? pick(pv, "homeStarter", "home.starter") : pick(pv, "awayStarter", "away.starter");
      const opP = myHome ? pick(pv, "awayStarter", "away.starter") : pick(pv, "homeStarter", "home.starter");
      T.game.homeSP = str(pick(myP, "playerName", "name"), T.game.homeSP);
      T.game.homeSPline = starterLine({ ...(myP || {}), ...(pick(myP, "currentSeasonStats", "stats", "season") || {}) });
      T.game.awaySP = str(pick(opP, "playerName", "name"), T.game.awaySP);
      T.game.awaySPline = starterLine({ ...(opP || {}), ...(pick(opP, "currentSeasonStats", "stats", "season") || {}) });
      /* 선발 카드: 카운팅 기록 수집 → 자체 계산 (FIP·K/9·kwERA·K-BB% 등) */
      const st = pick(myP, "currentSeasonStats", "stats", "season") || {};
      const rawP = {
        IP: pick(st, "inn", "ip", "inning") ?? pick(myP, "inn", "ip"),
        H: pick(st, "h", "hit"), HR: pick(st, "hr"), BB: pick(st, "bb"), HBP: pick(st, "hbp", "hp"),
        SO: pick(st, "so", "kk"), ER: pick(st, "er"), R: pick(st, "r"), TBF: pick(st, "tbf", "bf", "pa"),
        QS: pick(st, "qs"), ERA: pick(st, "era") ?? pick(myP, "era"), WHIP: pick(st, "whip")
      };
      const cp = computePitcher(rawP, lg);
      T.pitchers.starter = { ...T.pitchers.starter,
        name: T.game.homeSP, hand: str(pick(myP, "hand", "throwsHand"), T.pitchers.starter.hand),
        key: cp.key, detail: cp.detail, adv: cp.adv,
        vsKey: { ERA: s2(pick(myP, "vsTeamStats.era", "matchup.era")), WHIP: "—", "K/9": "—" },
        vsDetail: Object.fromEntries(Object.keys(cp.detail).map(k => [k, "—"])),
        vsAdv: Object.fromEntries(Object.keys(cp.adv).map(k => [k, "—"])),
        mix: [["구종 구사율 — 트래킹 데이터 미공개", 100, "#3E4E48"]],
        note: `${oppName} 상대 등판 — 계산 지표(FIP·kwERA·K-BB%)는 시즌 기록 기반 자체 산출`
      };
    ...
    /* 진행 가능성 (기상청 + 에어코리아) */
    try {
      const loc = stadiumInfo(stadium || T.game.venue);
    ...
    /* 시즌 상대전적 (시즌 일정 재사용) */
    try {
      const h = seasonGames ? computeH2H(seasonGames, code, oppCode) : null;
      if (h) {
        T.h2h.my = fresh.teams[teamKey].short;
        T.h2h.op = T.game.oppShort;
        T.h2h.w = h.w; T.h2h.l = h.l; T.h2h.d = h.d;
        T.h2h.recent = h.recent;
        T.h2h.notes = [`시즌 상대전적 ${h.w}승 ${h.d}무 ${h.l}패`, `최근 맞대결 ${h.recent.join(" ")}`];
        sources[`h2h_${teamKey}`] = "ok";
      } else sources[`h2h_${teamKey}`] = "맞대결 없음 — 이전 유지";
...
export async function runBatch2({ force = false } = {}) {
  ...
  for (const teamKey of Object.keys(TEAMCODE)) {
    ...
    try {
      ...
      const myHome = game.homeTeamCode === code;
      const raw = myHome ? pick(pv, "homeTeamLineUp", "lineup.home", "homeLineup") : pick(pv, "awayTeamLineUp", "lineup.away", "awayLineup");
      const list = Array.isArray(raw) ? raw : (Array.isArray(raw?.batters) ? raw.batters : null);
      if (!list || list.length < 9) throw new Error("라인업 미발표 또는 9인 미만");
      const batters = list.slice(0, 9).map((e, i) => mapBatter(e, i + 1, lg)).filter(Boolean);
      if (batters.length !== 9) throw new Error("라인업 매핑 실패");
      fresh.teams[teamKey].batters = batters;
      updatedAny = true;
      sources[`lineup_${teamKey}`] = "ok";
