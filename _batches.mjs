...
import { TEAMCODE, NAVER2CODE, getGames, findGame, isHome, getPreview, num, str, s2, starterLine, mapStarter, mapLineup, standingStr, h2hFromPreview, recentFromPreview } from "./collectors/naver.mjs";
...
export async function runBatch2({ force = false } = {}) {
  ...
  for (const teamKey of Object.keys(TEAMCODE)) {
    ...
    try {
      ...
      const batters = mapLineup(lineup);
      if (!batters) throw new Error("라인업 미발표 또는 9인 미만");
